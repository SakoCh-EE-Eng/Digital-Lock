...
bool Are_All_LEDs_On(void)
{
    // Mask the lower 4 bits (LED0 - LED3)
    uint8_t led_status = GPIOB->DATA & 0x0F;

    // Check if all 4 are ON (i.e., bits 0-3 are set)
    return (led_status == 0x0F);
}
